...
- v1 单个 parent task 同一时刻只支持一个远端 tool call。第一个 `INTERRUPTED(RemoteAgentInterrupt)` 会中断并关闭本地 result stream；同一轮推理中出现多个并行远端 tool call 时返回明确错误，不做并发 fan-out。
...
本地 LLM 选择远端 tool
  ...
  -> OpenJiuwenStreamAdapter 映射为 AgentExecutionResult.INTERRUPTED(RemoteAgentInterrupt)
...
```text
Type = OUTPUT / COMPLETED / FAILED / INTERRUPTED
AgentExecutionResult.interrupted(RemoteInvocation remoteInvocation)
InterruptPayload = UserInputInterrupt / RemoteAgentInterrupt
...
现有 `INTERRUPTED(UserInputInterrupt)` 继续表示普通本地 input-required；远端 tool call 使用同一个顶层 `INTERRUPTED`，但携带强类型 `RemoteAgentInterrupt` payload，避免把 remote agent id、toolCallId、arguments、parent task/context 塞进 prompt 字符串。v1 不单独新增 `RemoteAgentInvocationIntent.java`。
...
INTERRUPTED(UserInputInterrupt)   -> 写 parent input-required，停止消费
INTERRUPTED(RemoteAgentInterrupt) -> 返回 remote invocation decision，停止消费
未知类型           -> 抛 IllegalStateException
```

`INTERRUPTED(RemoteAgentInterrupt)` 必须停止继续消费本地 handler stream，不能让后续 `COMPLETED` 抢先写入 parent task。实现上不能继续使用 `Stream.forEach(...)`；必须使用可短路循环，例如在 `try-with-resources` 中通过 `Iterator` 消费 result，遇到 `COMPLETED`、`FAILED` 或 `INTERRUPTED` 后 `break`，并确保 raw stream 与 adapted stream 都被关闭。
...
本地 stream 的生命周期和 `AgentEmitter` 生命周期不同。`INTERRUPTED(RemoteAgentInterrupt)` 后可以关闭第一段本地 stream，但只要 `A2aAgentExecutor.execute(...)` 方法尚未返回，当前 `AgentEmitter` 仍可用于 parent task 投影。SDK 不应因为第一段本地 stream 结束就把 parent task 视为终态；终态只由 router/projector 显式调用 `complete/fail/requiresInput/cancel` 决定。
...
| `boot/RemoteAgentProperties.java` | 只定义 `agent-runtime.remote-agents[].url` 的配置绑定模型，并提供过滤空 URL 后的 `urls()` 视图；不创建 bean、不拉 card、不装配远端调用组件。 | 新增配置文件读取对象，远端 URL 仍是 list，支持一个或多个远端 runtime。 |
...
| `engine/spi/AgentExecutionResult.java` | 框架中立的 handler 执行结果模型。只表达结果类型和必要 payload，不依赖 A2A SDK、OpenJiuwen 类型或 service 层类型。 | 统一使用 `INTERRUPTED` 表达中断；新增 `InterruptPayload`、`UserInputInterrupt`、`RemoteAgentInterrupt` 和嵌套 `RemoteInvocation` record；普通本地 input-required 与远端调用通过 payload 区分。 |
...
| `engine/openjiuwen/OpenJiuwenStreamAdapter.java` | OpenJiuwen result/interrupt 到 `AgentExecutionResult` 的适配器。只解析 OpenJiuwen 结果结构并输出框架中立结果；不调用远端服务，不写 A2A task。 | 识别 OpenJiuwen remote interrupt marker，构造 `AgentExecutionResult.interrupted(remoteInvocation)`；普通 interrupt 仍映射为 `interrupted(prompt)`。 |
...
| `engine/a2a` | `A2aAgentExecutor` 内部 result 消费循环遇到 `INTERRUPTED(RemoteAgentInterrupt)` 后返回 remote invocation decision、停止继续消费本地 stream，并关闭 raw/adapted stream。 |

...
> **Demotion note (added 2026-05-26 rc55 per [ADR-0143](../../adr/0143-review-log-demotion-l1-canonical-move.yaml))**: This file is a HISTORICAL AUTHORING RECORD of the rc53 Wave 1-6 design process. The canonical L1 4+1 source has moved to `docs/L1/agent-service/{README,scenarios,logical,process,physical,development,spi-appendix}.md` per ADR-0143. Where this file and the canonical per-view files disagree, the canonical per-view files win. The rc55 audit identified that promoting a freeze-marked review log to "canonical L1 4+1 source" violated Rule G-1.a's L0/L1/L2 view discipline (registered as `F-l1-canonical-source-in-interaction-log` in `docs/governance/recurring-defect-families.yaml`); ADR-0143 closes the family by moving the canonical source out of `docs/logs/reviews/` into `docs/L1/`. This file remains read-only per `docs/governance/logs-folder-policy.md`; the demotion note is itself a permitted historical-marker addition.

> **Historical-artifact freeze marker (added 2026-05-26 rc53-wave-8 closure)**: This file is the canonical L1 4+1 source authored across Waves 1-6 of the rc53 wave. After Wave 8 closure (commit referenced in `docs/logs/releases/2026-05-26-rc53-agent-service-l1-4plus1-rewrite-closure.en.md`), this file is **read-only** per `docs/governance/logs-folder-policy.md`. Further edits flow as new review drafts under `docs/logs/reviews/`. Numeric values inside this document are point-in-time snapshot evidence; the live `agent-service/ARCHITECTURE.md` §0.5 carries the forward-compatible authority pointer. **rc55 update (per the demotion note above)**: §0.5's forward-compatible authority pointer now references `docs/L1/agent-service/` instead of this review file.

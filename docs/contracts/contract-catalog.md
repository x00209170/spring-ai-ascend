...
**Active SPI interfaces (28 total):**
...
| ~~`EnginePort`~~ | ~~`agent-bus`~~ | ~~`com.huawei.ascend.bus.spi.engine`~~ | **pending retirement** — EnginePort/dual-mode contract mechanism superseded by ADR-0160 `AgentDriver` seam; source retained pending R1 cleanup wave |
| ~~`ExecutionContext`~~ | ~~`agent-bus`~~ | ~~`com.huawei.ascend.bus.spi.engine`~~ | **pending retirement** — neutral engine correlation surface; superseded by ADR-0160; source retained pending R1 cleanup wave |
...
| ~~`AgentResultAdapter`~~ | ~~`agent-runtime`~~ | ~~`com.huawei.ascend.runtime.dispatch.spi`~~ | **retired** — AgentHandler/AgentResultAdapter dispatch SPI retired by ADR-0160; replaced by `OutputConverter` |
...
| ~~`ExecutorAdapter`~~ | ~~`agent-runtime`~~ | ~~`com.huawei.ascend.runtime.executor.spi`~~ | **retired** — Layer 5a ExecutorAdapter design_only spec retired by ADR-0160 |
...
**SPI count by module (ADR-0160 neutral core baseline; 47 original − 19 agent-middleware retired − 3 EnginePort/ExecutorAdapter/AgentResultAdapter retired + 2 AgentDriver/OutputConverter added = 28):**

| Module | SPI interfaces |
|---|---|
| `agent-service` | 0 — serviceization façade skeleton |
| `agent-runtime` | 20 (`RunRepository`, `GraphMemoryRepository`, `ResilienceContract`, `SkillCapacityRegistry`, `ContextProjector`, `TaskStateStore`, `Agent`, `AgentRegistry`, `AgentDriver`, `OutputConverter`, `EngineDispatchApi`, `EngineQueueGateway`, `EngineCommandConsumer`, `AccessLayerClient`, `TaskControlClient`, `Planner`, `PlatformChatClient`, `PlatformToolCallback`, `PlatformMemoryProvider`, `PlatformRetriever`) |
| `agent-bus` | 7 (`IngressGateway`, `S2cCallbackTransport`, `ReflectionEnvelopeRouter`, `FederationGateway`, `Checkpointer`, `Orchestrator`, `DefinitionResolver`) |
...
`com.huawei.ascend:spring-ai-ascend-dependencies:0.1.0-SNAPSHOT` — active reactor modules (5 per `architecture-status.yaml#repository_counts.reactor_modules`):
...
| `agent-service` | compute_control | Enterprise serviceization façade (skeleton) — registration/discovery driving runtime-built Agent instances via agent-runtime (ADR-0159) |
| `agent-runtime` | compute_control | Run-owning runtime SDK: neutral execution core (`AgentDriver` SPI + `RunCoordinator` + `OutputConverter`) + `RunEvent` stream + framework adapters (openjiuwen, Dify) + engine dispatch + session/task-control + access (A2A) + bootable runtime app — ADR-0160 |
| `agent-runtime-spring-boot-starter-a2a` | compute_control | Spring Boot auto-config starter for microservice-mode A2A deployment — wires `AgentDriver` beans to A2A serving |
| `agent-bus` | bus_state | Cross-plane control surfaces: `bus.spi.ingress.IngressGateway` (ADR-0089) + `bus.spi.s2c.S2cCallbackTransport` (ADR-0074 + ADR-0088). Workflow primitives W2 per ADR-0050 |
...
- **ADR-0160** **retired** the EnginePort/ExecutorAdapter/dual-mode contract mechanism and established the neutral `AgentDriver` + `RunCoordinator` + `OutputConverter` execution seam as the current engine model. The `InProcessEnginePort` and `agent-bus.bus.spi.engine` package were removed.

...
| `EngineDispatchSpi` | `agent-service` | `com.huawei.ascend.service.engine.spi` | (internal) design_only — async dispatch entry for task-centric-control to enqueue Agent execution; intra-service contract; design authority: L1 engine model design doc |

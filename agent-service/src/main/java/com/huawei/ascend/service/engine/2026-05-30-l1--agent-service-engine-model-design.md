...
spi/   出站/扩展接口：由 engine 定义，供外部或插件实现
```
...
| 方向 | 入站（inbound / provided） | 出站或扩展（outbound / extension） |
| 本模块示例 | `EngineDispatchApi` | `AgentHandler`、`EngineQueueGateway`、`TaskControlClient`、`AccessLayerClient` |
...
基础设施端口 EngineQueueGateway 由 engine 定义、被队列实现接入，是 SPI。
出站客户端 TaskControlClient / AccessLayerClient 由 engine 定义、被对应服务的适配器实现，是 SPI。
...
> - `spi/` 出站与扩展接口（engine 定义，外部或插件实现）：`AgentHandler`（插件扩展点）、`EngineQueueGateway` + `EngineCommandConsumer`（队列基础设施端口）、`TaskControlClient` / `AccessLayerClient`（出站客户端端口）。
> - `dispatch/`、`queue/`（工厂与订阅者）、`adapter/`、`config/` 为 engine 内部实现，既非 API 也非 SPI。`AgentHandlerRegistry` 由 engine 自身实现并自用，属内部接口，保留在 `dispatch/`。
> - `model/`、`event/`、`handler/AgentExecutionContext` 为数据/上下文载体，按 Rule R-D.d 不放入 `spi/` 包。
> - `TaskControlClient` / `AccessLayerClient` 的具体实现为出站适配器，落在 `adapter/`。
...
| `EngineQueueGateway` | SPI（基础设施端口） | `engine.spi` | engine → 队列实现 | §7.1 |
| `EngineCommandConsumer` | SPI（回调） | `engine.spi` | engine → engine（订阅回调载体） | §7.4 |
| `TaskControlClient` | SPI（出站端口） | `engine.spi` | engine → task-centric-control 适配器 | §11.1 |
| `AccessLayerClient` | SPI（出站端口） | `engine.spi` | engine → access-layer 适配器 | §11.2 |

> v1.1 错误：曾将入站入口 `EngineDispatchSpi` 及其请求/状态载体整体归入"SPI 定义"。
> 入站入口由 engine 实现、被 task-centric-control 调用，本质是 **API**。
> v1.2 已将其重命名为 `EngineDispatchApi` 并迁入 `api/`；真正的 SPI（扩展点与出站端口）
> 集中在 §7、§9、§11，并在本表统一登记。
...
不直接执行 Agent。
不直接返回真实执行状态。
真实执行状态通过 TaskControlClient（SPI 出站端口）回写。
```
...
```text
agent-service/src/main/java/com/huawei/ascend/service/engine/spi/EngineQueueGateway.java
```
...
```text
SPI（基础设施端口）：由 engine 定义，由队列实现接入。
...
```text
agent-service/src/main/java/com/huawei/ascend/service/engine/spi/EngineCommandConsumer.java
```
...
```text
agent-service/src/main/java/com/huawei/ascend/service/engine/spi/TaskControlClient.java
```
...
```text
SPI（出站端口）：由 engine 定义，由 task-centric-control 适配器实现。
...
```text
agent-service/src/main/java/com/huawei/ascend/service/engine/spi/AccessLayerClient.java
```
...
```text
SPI（出站端口）：由 engine 定义，由 access-layer 适配器实现。
...
Parent Agent
  ...
  -> EngineDispatchApi.enqueueExecution(child agentId, child input)
  -> parent task enters WAITING_CHILD_AGENT
  -> child task completes
  -> task-centric-control calls EngineDispatchApi.enqueueResume(parent scope, AGENT_CALL_RESULT)
  -> parent resumes
```
...
spi/TaskControlClient.java
spi/AccessLayerClient.java
```
